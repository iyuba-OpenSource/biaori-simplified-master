...
public class AnswerAdapter extends BaseQuickAdapter<JpWord, BaseViewHolder> {
    ...
    public AnswerAdapter(int layoutResId, @Nullable List<JpWord> data) {
    ...
    protected void convert(@NonNull BaseViewHolder helper, JpWord item) {
        ...
        if (choosePosition == -1 && tPosition == -1) {
            ...
            if(choosePosition == tPosition&&tPosition == helper.getAdapterPosition()){

                helper.setBackgroundRes(R.id.answer_tv_content, R.mipmap.bg_answer_right);
            }else if(choosePosition != tPosition&&choosePosition == helper.getAdapterPosition()){

                helper.setBackgroundRes(R.id.answer_tv_content, R.mipmap.bg_answer_wrong);
            }else if(choosePosition != tPosition&&tPosition == helper.getAdapterPosition()){
